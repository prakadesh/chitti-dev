...
namespace ClipEnhancer.Services;
...
public class ClipboardMonitorService
{
    ...
    private List<string> ExtractTags(string text)
    {
        ...
        if (lowered.Contains("@caret"))
        {
            // Extract the custom prompt after @caret
            var caretIndex = lowered.IndexOf("@caret");
            if (caretIndex >= 0)
            {
                var prompt = text.Substring(caretIndex + "@caret".Length).Trim();
                if (!string.IsNullOrEmpty(prompt))
                {
                    tags.Add($"@caret {prompt}");
                }
            }
        }
